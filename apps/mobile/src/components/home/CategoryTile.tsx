...
export function CategoryTile({ title, image, index, onPress }: { title: string; image: string; index: string; onPress?: () => void }) {
  return (
    <Pressable onPress={onPress} style={styles.tile}>
      <Image source={image} style={styles.image} contentFit="cover" transition={180} />
      <View style={styles.overlay} />
      <View style={styles.top}><Text style={styles.index}>{index}</Text><ArrowUpRight size={18} color={colors.white} strokeWidth={1.5} /></View>

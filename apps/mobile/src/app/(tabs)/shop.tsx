...
export default function ShopScreen() {
  ...
  const sortedItems = (() => {
    ...
    const priceOf = (p: (typeof items)[number]) => Math.min(...p.variants.map((v) => Number(v.price)));
    return [...items].sort((a, b) => sort === 'price-asc' ? priceOf(a) - priceOf(b) : priceOf(b) - priceOf(a));
  })();
  return <View style={styles.container}><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.scroll, { paddingTop: insets.top + spacing.md }]}>

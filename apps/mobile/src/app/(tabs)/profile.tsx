import { Bell, ChevronRight, Heart, LogOut, MapPin, Package, Settings, ShieldCheck, UserRound } from 'lucide-react-native';
...
export default function ProfileScreen() {
  ...
  return <View style={styles.container}><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.content, { paddingTop: insets.top + spacing.md }]}>
    ...
    <Section title="PERSONAL"><AccountRow icon={UserRound} title="Personal information" subtitle="Name, email and profile details" onPress={() => router.push('/profile/edit')}/><AccountRow icon={MapPin} title="Addresses" subtitle="Manage delivery addresses" onPress={() => router.push('/profile/addresses')}/><AccountRow icon={ShieldCheck} title="Security" subtitle="Password and account security" onPress={() => router.push('/profile/security')}/></Section>
    <Section title="PREFERENCES"><AccountRow icon={Settings} title="Settings" subtitle="Notifications, privacy and preferences" onPress={() => router.push('/profile/settings')}/></Section>
    <Pressable style={styles.logout} onPress={async()=>{await logout();router.replace('/(auth)/login')}}><LogOut size={17} color={colors.danger}/><Text style={styles.logoutText}>Sign out</Text></Pressable>

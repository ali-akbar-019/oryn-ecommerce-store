...
type ApiCart = {
  ...
  items: Array<{
    ...
    variantId: string;
    quantity: number;
    product: { id: string; name: string; images: { url: string; sortOrder: number }[] };
    variant: { id: string; price: string | number; stockQuantity: number; attributes: Record<string, unknown>; inventory?: { quantity: number } | null };
  }>;
};
...
function attributesToDisplay(attributes: Record<string, unknown>) {
  const color = Object.entries(attributes).find(([key]) => key.toLowerCase() === 'color')?.[1];
  const size = Object.entries(attributes).find(([key]) => key.toLowerCase() === 'size')?.[1];
  return { color: color == null ? undefined : String(color), size: size == null ? undefined : String(size) };
}
...
function mapCart(cart: ApiCart): CartItem[] {
  return cart.items.map((item) => {
    ...
    return {
      ...
      image: [...item.product.images].sort((a, b) => a.sortOrder - b.sortOrder)[0]?.url ?? '',
...
export const useCartStore = create<CartState>((set) => ({
  ...
  hydrate: async () => {
    ...
    try {
      ...
      set({ error: error instanceof Error ? error.message : 'Unable to load your cart.', hydrated: true });
    } finally {
      set({ loading: false });
    }
  },
  addVariant: async (product, variantId, quantity) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Unable to add this item to your bag.';
      set({ error: message });
      throw error;
    } finally {
      set({ loading: false });
  ...
  updateQuantity: async (key, quantity) => {
    ...
    try {
      ...
      set({ error: error instanceof Error ? error.message : 'Unable to update your bag.' });
      throw error;
  ...
  removeItem: async (key) => {
    ...
    try {
      ...
      set({ error: error instanceof Error ? error.message : 'Unable to remove this item.' });
      throw error;
    } finally { set({ loading: false }); }
  },

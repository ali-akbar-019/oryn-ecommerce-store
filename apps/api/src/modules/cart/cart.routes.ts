...
export const cartRouter = Router() as Router;
...
cartRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existing = await prisma.cartItem.findFirst({
      where: { cart: { userId }, variantId: input.variantId }
    });

    const nextQuantity = (existing?.quantity ?? 0) + input.quantity;
    if (nextQuantity > available) {
      throw new AppError(409, 'INSUFFICIENT_STOCK', 'The selected quantity is not available.');
    }
...
cartRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const item = await prisma.cartItem.findFirst({
      where: { id: asStr(req.params.id), cart: { userId } },

...
function App(){return <BrowserRouter><Routes><Route path="/login" element={<Login/>}/><Route element={<Protected/>}><Route index element={<Dashboard/>}/><Route path="products/new" element={<ProductEditor/>}/><Route path="products/:id" element={<ProductEditor/>}/><Route path="orders/:id" element={<OrderWorkspace/>}/><Route path="customers/:id" element={<CustomerWorkspace/>}/>{resources.map(resource=><Route key={resource} path={resource} element={['products','categories','inventory','orders','customers'].includes(resource)?<ManagementPage resource={resource as any}/>:operationResources.includes(resource)?<OperationsWorkspace resource={resource as any}/>:<ResourcePage resource={resource}/>}/>}<Route path="settings" element={<Settings/>}/></Route></Routes></BrowserRouter>}
